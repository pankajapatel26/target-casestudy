...
import com.casestudy.quizservice.constants.SecurityConstants;
...
public class JwtTokenUtil implements Serializable {
    ...
    public String generateToken(CustomUserDeatils user) {
        return doGenerateToken(user);
    }

    private String doGenerateToken(CustomUserDeatils user) {

        Claims claims = Jwts.claims().setSubject(user.getUsername());
        claims.put("scopes", Arrays.asList(new SimpleGrantedAuthority("ROLE_ADMIN")));
        claims.put("firstName", user.getFirstName());
        claims.put("firstName", user.getLastName());
        claims.put("firstName", user.getPhoneNumber());

        return Jwts.builder()
                .setClaims(claims)
                .setIssuer("http://casestudy.com")
